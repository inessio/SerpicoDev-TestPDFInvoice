
import jsPDF from 'jspdf'
import 'jspdf-autotable'

 const  generateInvoice = (cookiePackage) => {
    var doc = new jsPDF('p', 'pt','A4')
    var res = doc.autoTableHtmlToJson(document.getElementById("my-table"))
    const logoData = 'data:image/png;base64,'
    doc.setFontSize(40)
    doc.setTextColor(9,46,79)
    doc.text('INVOICE',350,80)
    doc.setFillColor(68,128,182);
    doc.rect(8, 3, 580, 20, 'F');
    doc.setTextColor(14,13,14)
    doc.setFontSize(14)
    doc.addImage(logoData, 'PNG',30,45, 60, 60)
    doc.text('SerpiCookies',95,70)
    doc.text('Website: ' + cookiePackage.website,95,90)
    doc.text('Date: ' + new Date().toLocaleString(),30,130)
    doc.text('Client: ' + cookiePackage.clientName, 30, 150)
    doc.autoTable(res.columns, res.data, { margin: {top: 180}})
    if(cookiePackage.comment !== ''){
      doc.setFontSize(14)
      doc.text('Comment:',40,doc.previousAutoTable.finalY + 50)
      doc.setFontSize(12)
      doc.text(cookiePackage.comment, 100,doc.previousAutoTable.finalY + 70)
    }
    // doc.splitTextToSize(comments, 80)
    doc.setFillColor(68,128,182);
    doc.rect(8, 818, 580, 20, 'F');
    doc.save("invoice.pdf");
}

export default generateInvoice